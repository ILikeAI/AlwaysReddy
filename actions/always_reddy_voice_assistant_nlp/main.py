...
import prompt
from nlp_manager import get_nlp_context
class AlwaysReddyVoiceAssistant_nlp(BaseAction):
    """Action for handling voice assistant functionality."""
    def setup(self):
        self.last_message_was_cut_off = False

        if config.RECORD_WITH_NLP_HOTKEY:
            self.AR.add_action_hotkey(
                config.RECORD_WITH_NLP_HOTKEY, 
            ...
            print(f"'{config.RECORD_WITH_NLP_HOTKEY}': Start/stop talking to voice assistant (press to toggle on and off, or hold and release)")
            if "+" in config.RECORD_WITH_NLP_HOTKEY:
                hotkey_start, hotkey_end = config.RECORD__WITH_NLP_HOTKEY.rsplit("+", 1)
                print(f"\tHold down '{hotkey_start}' and double tap '{hotkey_end}' to send clipboard content to AlwaysReddy")
            else:
                print(f"\tDouble tap '{config.RECORD_WITH_NLP_HOTKEY}' to send clipboard content to AlwaysReddy")
    ...
    def handle_default_assistant_response(self):
        ...
        try:
            ...
            if not self.AR.stop_action and message:
                ...
                # Handle potential NLP context requests for weather, news and search
                nlp_context = get_nlp_context(self.AR,message)
                if nlp_context:
                    new_message['content'] = nlp_context
                ...
                end_time = time.time()
                if self.AR.verbose: print(f"Execution time: {end_time - start_time:.1f} seconds")
